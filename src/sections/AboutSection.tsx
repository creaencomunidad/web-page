...
const AboutSection: React.FC<AboutSectionProps> = ({
	data: { title, description, imgPath, buttonText, buttonIcon, buttonHref, aboutPieces }
}) => {
	return (
		<div
			id="quienes-somos"
		...
		>
			<div className="mb-10 flex flex-row flex-wrap items-center justify-center">
				<div
					style={{
						width: '500px',
						display: 'flex',
				...
				>
					...
					<ExternalButton data={{ label: buttonText, href: buttonHref, icon: buttonIcon }} />
				</div>
			...
			{aboutPieces.map((piece, index) => {
				return (
					...
					>
						<h3 className="title-font crea-dark-blue-text text-center">{piece.title}</h3>
						<p
			...
			<div className="relative bottom-6 flex flex-col items-center">
				<ExternalButton
					data={{ href: 'https://blog.creaencomunidad.com', label: '¡Explora nuestro Blog!' }}
				/>
			</div>
		</div>
	)
